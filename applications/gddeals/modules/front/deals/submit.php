...
class _submit extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$form = new \IPS\Helpers\Form( 'gddeals_submit', 'gddeals_submit_button' );

		$form->add( new \IPS\Helpers\Form\Node( 'gddeals_f_category', NULL, TRUE, [
			'class'           => 'IPS\\gddeals\\Category',
			'permissionCheck' => 'add',
			'subnodes'        => FALSE,
		] ) );

		$form->add( new \IPS\Helpers\Form\Text( 'gddeals_f_title', NULL, TRUE, [
			'maxLength' => 255,
		] ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'gddeals_f_posttype', 'product', TRUE, [
			'options' => $postTypes,
		] ) );

		$form->add( new \IPS\Helpers\Form\Text( 'gddeals_f_retailer_name', NULL, TRUE, [
			'maxLength' => 150,
		] ) );
		...
		$form->add( new \IPS\Helpers\Form\Select( 'gddeals_f_retailer_type', 'online', FALSE, [
			'options' => $retailerTypes,
		] ) );

		$form->add( new \IPS\Helpers\Form\Text( 'gddeals_f_store_location', NULL, FALSE, [
			'maxLength' => 200,
		] ) );

		$form->add( new \IPS\Helpers\Form\Number( 'gddeals_f_deal_price', NULL, FALSE, [
			'decimals' => 2,
			'min'      => 0,
		] ) );

		$form->add( new \IPS\Helpers\Form\Number( 'gddeals_f_original_price', NULL, FALSE, [
			'decimals' => 2,
			'min'      => 0,
		] ) );

		$form->add( new \IPS\Helpers\Form\Url( 'gddeals_f_deal_url', NULL, FALSE ) );

		$form->add( new \IPS\Helpers\Form\Text( 'gddeals_f_promo_code', NULL, FALSE, [
			'maxLength' => 100,
		] ) );
		...
		$form->add( new \IPS\Helpers\Form\TextArea( 'gddeals_f_description', NULL, FALSE ) );

		$form->add( new \IPS\Helpers\Form\Date( 'gddeals_f_expires', NULL, FALSE ) );
		...
		\IPS\Output::i()->title  = $member->language()->addToStack( 'gddeals_submit_title' );
