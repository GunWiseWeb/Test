<?php
/**
 * @brief  GD Deals — upgrade 1.0.63 (CORRECTED template seed — v1.0.61 broke globalTemplate).
 *
 * Rule #79 — exactly ONE upg_* dir per app. Self-contained.
 * Rule #27 — dual class wrapper, guard header.
 *
 * WHAT SHIPS IN 1.0.63 — CORRECTION OF 1.0.61
...
namespace IPS\gddeals\setup\upg_10063;
		...
		if ( is_dir( $root ) )
		{
			try
			{
				...
				foreach ( $it as $f )
				{
					...
					{
						try { \IPS\Log::log( 'upg_10063 tpl (' . $name . '): ' . $e->getMessage(), 'gddeals_upg_10063' ); } catch ( \Throwable ) {}
					}
				}
			}
			catch ( \Throwable $e )
			{
				try { \IPS\Log::log( 'upg_10063 tpl loop: ' . $e->getMessage(), 'gddeals_upg_10063' ); } catch ( \Throwable ) {}
			}
		}
