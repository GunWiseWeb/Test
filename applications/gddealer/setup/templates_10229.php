...
<div class="gdDealerWrapper" style="width:100%;max-width:1446px;margin:0 auto;padding:0 24px;box-sizing:border-box;--gd-dealer-brand:{$data['dealer']['brand_color']}">
	...
	<div class="ipsProfile ipsProfile--profile" style="display:flex;gap:24px;flex-wrap:wrap">
		...
		<div class="ipsProfile__main" style="flex:1 1 0;min-width:0">
			...
			{{if isset($data['deals']) && count($data['deals']) > 0}}
			<div class="ipsBox i-margin-bottom_block" style="background:#fff;border:1px solid var(--i-border-color,#e0e0e0);border-radius:8px;margin-bottom:24px">
				<h3 class="ipsBox__header" style="margin:0;padding:14px 18px;border-bottom:1px solid var(--i-border-color,#f0f0f0);font-size:1em;font-weight:700">
					<i class="fa-solid fa-tags" aria-hidden="true" style="color:#16a34a;margin-right:6px"></i> Active Deals
					<span style="font-size:0.75em;font-weight:400;color:#666">({expression="count($data['deals'])"})</span>
				</h3>
				<div style="padding:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px">
					{{foreach $data['deals'] as $deal}}
					<div style="border:1px solid var(--i-border-color,#e8e8e8);border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
						{{if $deal['image_url']}}
						<div style="height:160px;overflow:hidden;background:#f8f9fa;display:flex;align-items:center;justify-content:center">
							<img src="{$deal['image_url']}" alt="" loading="lazy" style="max-width:100%;max-height:100%;object-fit:contain">
						</div>
						{{endif}}
						<div style="padding:12px;flex:1;display:flex;flex-direction:column">
							{{if $deal['brand']}}
							<div style="font-size:0.72em;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:4px">{$deal['brand']}</div>
							{{endif}}
							<div style="font-weight:700;font-size:0.9em;margin-bottom:8px;line-height:1.3">{$deal['product_title']}</div>
							<div style="display:flex;align-items:baseline;gap:8px;margin-bottom:4px">
								{{if $deal['deal_price']}}
								<span style="font-size:1.15em;font-weight:800;color:#16a34a">{$deal['deal_price']}</span>
								{{endif}}
								{{if $deal['regular_price']}}
								<span style="font-size:0.85em;color:#9ca3af;text-decoration:line-through">{$deal['regular_price']}</span>
								{{endif}}
								{{if $deal['pct_off']}}
								<span style="font-size:0.75em;font-weight:700;color:#dc2626;background:#fee2e2;padding:2px 6px;border-radius:4px">{$deal['pct_off']}</span>
								{{endif}}
							</div>
							{{if $deal['title']}}
							<div style="font-size:0.82em;color:#6b7280;margin-top:4px">{$deal['title']}</div>
							{{endif}}
							{{if $deal['end_date']}}
							<div style="font-size:0.75em;color:#9ca3af;margin-top:auto;padding-top:8px">Ends {$deal['end_date']}</div>
							{{endif}}
						</div>
					</div>
					{{endforeach}}
			...
			<div class="ipsBox i-margin-bottom_block" style="background:#fff;border:1px solid var(--i-border-color,#e0e0e0);border-radius:8px;margin-bottom:24px">
				<h3 class="ipsBox__header" style="margin:0;padding:14px 18px;border-bottom:1px solid var(--i-border-color,#f0f0f0);font-size:1em;font-weight:700">
					<i class="fa-solid fa-ticket" aria-hidden="true" style="color:#2563eb;margin-right:6px"></i> Active Coupons
					<span style="font-size:0.75em;font-weight:400;color:#666">({expression="count($data['coupons'])"})</span>
				</h3>
				<div style="padding:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px">
					{{foreach $data['coupons'] as $coupon}}
					<div style="border:1px dashed #16a34a;border-radius:8px;padding:14px 16px;background:#f0fdf4;display:flex;align-items:flex-start;gap:12px">
						<div style="flex-shrink:0;background:#16a34a;color:#fff;border-radius:6px;padding:8px 12px;font-weight:800;font-size:0.9em;text-align:center;min-width:60px">
							{$coupon['discount_display']}
						</div>
						<div style="flex:1;min-width:0">
							<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
								<code style="background:#dcfce7;color:#166534;padding:2px 8px;border-radius:4px;font-size:0.85em;font-weight:700;letter-spacing:0.05em">{$coupon['code']}</code>
							</div>
							{{if $coupon['description']}}
							<div style="font-size:0.85em;color:#374151;margin-bottom:4px">{$coupon['description']}</div>
							{{endif}}
							{{if $coupon['min_purchase']}}
							<div style="font-size:0.75em;color:#6b7280">Min. purchase: {$coupon['min_purchase']}</div>
							{{endif}}
							{{if $coupon['terms']}}
							<div style="font-size:0.72em;color:#9ca3af;margin-top:4px;font-style:italic">{$coupon['terms']}</div>
							{{endif}}
							{{if $coupon['expiry']}}
							<div style="font-size:0.72em;color:#9ca3af;margin-top:4px">Expires {$coupon['expiry']}</div>
							{{endif}}
						</div>
					</div>
					{{endforeach}}
			...
			{{if isset($data['listings']) && count($data['listings']) > 0}}
			<div class="ipsBox i-margin-bottom_block" style="background:#fff;border:1px solid var(--i-border-color,#e0e0e0);border-radius:8px;margin-bottom:24px">
				<h3 class="ipsBox__header" style="margin:0;padding:14px 18px;border-bottom:1px solid var(--i-border-color,#f0f0f0);font-size:1em;font-weight:700">
					<i class="fa-solid fa-box-open" aria-hidden="true" style="color:#1e40af;margin-right:6px"></i> Product Listings
					<span style="font-size:0.75em;font-weight:400;color:#666">({expression="count($data['listings'])"})</span>
				</h3>
				<div style="padding:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px">
					...
					<div style="border:1px solid var(--i-border-color,#e8e8e8);border-radius:8px;overflow:hidden;display:flex;flex-direction:column">
						...
						<div style="height:140px;overflow:hidden;background:#f8f9fa;display:flex;align-items:center;justify-content:center">
							<img src="{$listing['image_url']}" alt="" loading="lazy" style="max-width:100%;max-height:100%;object-fit:contain">
						</div>
						{{endif}}
						<div style="padding:10px 12px;flex:1;display:flex;flex-direction:column">
							{{if $listing['brand']}}
							<div style="font-size:0.7em;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:2px">{$listing['brand']}</div>
							{{endif}}
							<div style="font-weight:600;font-size:0.82em;margin-bottom:6px;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden">{$listing['title']}</div>
							<div style="margin-top:auto;display:flex;align-items:center;justify-content:space-between">
								<span style="font-weight:800;color:#1e40af;font-size:1em">{$listing['price_display']}</span>
								<span style="font-size:0.7em;font-weight:600;color:#16a34a;background:#dcfce7;padding:2px 6px;border-radius:4px">In Stock</span>
							</div>
							{{if $listing['product_url']}}
							<a href="{$listing['product_url']}" target="_blank" rel="noopener" style="display:block;text-align:center;margin-top:8px;font-size:0.8em;color:#2563eb;text-decoration:none;font-weight:600">View at store &rarr;</a>
							{{endif}}
						</div>
					</div>
					{{endforeach}}
