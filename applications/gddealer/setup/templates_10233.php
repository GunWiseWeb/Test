<?php
/**
 * v1.0.281 — dealerProfile overlay: IPS-style cover+sidebar profile layout
 */
...
<div class="gdProfile" style="--gd-dealer-brand:{$data['dealer']['brand_color']}">
	...
	<div class="gdProfile__cover">
		<div class="gdProfile__coverGradient"></div>
	</div>

	<div class="gdProfile__heroWrap">
		<div class="gdProfile__avatarWrap">
		...
		<div class="gdProfile__heroInfo">
			<div class="gdProfile__heroNameRow">
			...
			<div class="gdProfile__heroMeta">
				{{if $data['dealer']['address_city_state']}}
				<span class="gdProfile__heroMetaItem"><i class="fa-solid fa-location-dot" aria-hidden="true"></i> {$data['dealer']['address_city_state']}</span>
				{{endif}}
				<span class="gdProfile__heroMetaItem gdProfile__heroMetaItem--rating" style="{expression="'color:' . ($data['stats']['rating_color'] ?? '#1E40AF')"}">
					<span class="gdProfile__heroRatingScore">{$data['stats']['avg_overall']}</span>
					<span class="gdProfile__heroRatingStar">&#9733;</span>
					<span class="gdProfile__heroRatingCount">({expression="number_format($data['stats']['total'])"})</span>
				</span>
				<span class="gdProfile__heroMetaItem"><i class="fa-solid fa-box-open" aria-hidden="true"></i> {$data['dealer']['listing_count']} listings</span>
			</div>
		</div>
	</div>
	...
	<div class="gdProfile__body">

		<aside class="gdProfile__sidebar">
			{{if $data['dealer']['about']}}
			<div class="gdProfile__sidebarCard">
				<h3 class="gdProfile__sidebarCardTitle"><i class="fa-solid fa-store" aria-hidden="true"></i> About</h3>
				<div class="gdProfile__sidebarCardBody">{$data['dealer']['about']}</div>
			</div>
			{{endif}}

			<div class="gdProfile__sidebarCard">
				<h3 class="gdProfile__sidebarCardTitle"><i class="fa-solid fa-address-book" aria-hidden="true"></i> Contact</h3>
				<ul class="gdProfile__sidebarList">
					{{if $data['dealer']['public_phone']}}
					<li><i class="fa-solid fa-phone" aria-hidden="true"></i> <a href="tel:{$data['dealer']['public_phone']}">{$data['dealer']['public_phone']}</a></li>
					{{endif}}
					{{if $data['dealer']['public_email']}}
					<li><i class="fa-solid fa-envelope" aria-hidden="true"></i> <a href="mailto:{$data['dealer']['public_email']}">{$data['dealer']['public_email']}</a></li>
					{{elseif $data['dealer']['contact_email']}}
					<li><i class="fa-solid fa-envelope" aria-hidden="true"></i> <a href="mailto:{$data['dealer']['contact_email']}">Contact</a></li>
					{{endif}}
					{{if $data['dealer']['website_url']}}
					<li><i class="fa-solid fa-globe" aria-hidden="true"></i> <a href="{$data['dealer']['website_url']}" target="_blank" rel="noopener">Website</a></li>
					{{endif}}
					{{if $data['dealer']['address_public']}}
					<li><i class="fa-solid fa-map-pin" aria-hidden="true"></i> {$data['dealer']['address_line']}</li>
					{{endif}}
				</ul>
				{{if count($data['dealer']['socials']) > 0}}
				<div class="gdProfile__sidebarSocials">
					{{foreach $data['dealer']['socials'] as $social}}
					<a href="{$social['url']}" target="_blank" rel="noopener" class="gdProfile__sidebarSocialLink" title="{$social['network']}">
			...
			<div class="gdProfile__sidebarCard">
				<h3 class="gdProfile__sidebarCardTitle"><i class="fa-solid fa-shield-halved" aria-hidden="true"></i> Dealer Status</h3>
				<ul class="gdProfile__sidebarList gdProfile__sidebarList--compact">
					{{if $data['dealer']['verified']}}
					<li class="gdProfile__sidebarStatus gdProfile__sidebarStatus--verified"><i class="fa-solid fa-circle-check" aria-hidden="true"></i> FFL Verified</li>
					{{endif}}
			...
			<div class="gdProfile__sidebarCard gdProfile__sidebarCard--links">
				<a href="{$data['guidelines_url']}" class="gdProfile__sidebarLink"><i class="fa-solid fa-circle-info" aria-hidden="true"></i> Review Guidelines</a>
			</div>
		</aside>

		<div class="gdProfile__main">
			...
			<nav class="gdProfile__tabs">
				...
				<label for="gdTabListings" class="gdProfile__tab gdProfile__tab--listings">
					<i class="fa-solid fa-box-open" aria-hidden="true"></i> <span class="gdProfile__tabLabel">Listings</span> <span class="gdProfile__tabBadge">{expression="number_format($data['listings_total'])"}</span>
				</label>
				...
				<label for="gdTabInfo" class="gdProfile__tab gdProfile__tab--info">
					<i class="fa-solid fa-circle-info" aria-hidden="true"></i> <span class="gdProfile__tabLabel">Info</span>
				</label>
			</nav>
			...
			<div class="gdProfile__panel gdProfile__panel--listings">
				<div class="gdProfile__listingsToolbar">
					<form method="get" class="gdProfile__listingsSearch">
						...
						<button type="submit" class="gdProfile__listingsSearchBtn"><i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i></button>
			...
			<div class="gdProfile__panel gdProfile__panel--info">
				<div class="gdProfile__infoLayout">
					{{if $data['dealer']['has_hours'] || $data['dealer']['address_public'] || $data['dealer']['public_phone'] || $data['dealer']['public_email'] || $data['dealer']['website_url'] || count($data['dealer']['payment_methods']) > 0}}
					<aside class="gdProfile__infoSidebar">
						{{if $data['dealer']['has_hours']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-regular fa-clock" aria-hidden="true"></i> Hours</h3>
							<table class="gdProfile__hoursTable">
								{{foreach $data['dealer']['hours'] as $day}}
								<tr class="{{if $day['is_today']}}gdProfile__hoursToday{{endif}}">
									<td class="gdProfile__hoursDay">{$day['label']}</td>
									<td class="gdProfile__hoursTime">{{if $day['closed']}}Closed{{else}}{$day['open']} &ndash; {$day['close']}{{endif}}</td>
								</tr>
								{{endforeach}}
							</table>
						</div>
						...
						{{if $data['dealer']['address_public'] || $data['dealer']['public_phone'] || $data['dealer']['public_email'] || $data['dealer']['website_url']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-location-dot" aria-hidden="true"></i> Contact &amp; Location</h3>
							<ul class="gdProfile__infoList">
								{{if $data['dealer']['address_public']}}
								<li><i class="fa-solid fa-map-pin" aria-hidden="true"></i> {$data['dealer']['address_line']}</li>
								{{endif}}
								{{if $data['dealer']['public_phone']}}
								...
								{{endif}}
								{{if $data['dealer']['website_url']}}
								<li><i class="fa-solid fa-globe" aria-hidden="true"></i> <a href="{$data['dealer']['website_url']}" target="_blank" rel="noopener">{$data['dealer']['website_url']}</a></li>
								{{endif}}
							</ul>
						</div>
						{{endif}}
						...
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-credit-card" aria-hidden="true"></i> Payment Methods</h3>
							<div class="gdProfile__pmList">
								{{foreach $data['dealer']['payment_methods'] as $pm}}
								<span class="gdProfile__pmPill">{$pm['label']}</span>
								{{endforeach}}
							</div>
						</div>
						{{endif}}
					...
					<div class="gdProfile__infoMain">
						{{if $data['dealer']['about']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-store" aria-hidden="true"></i> About</h3>
							<div class="gdProfile__infoCardBody">{$data['dealer']['about']}</div>
						</div>
						{{endif}}

						{{if $data['dealer']['tagline']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-quote-left" aria-hidden="true"></i> Tagline</h3>
							<div class="gdProfile__infoCardBody">{$data['dealer']['tagline']}</div>
						</div>
						{{endif}}

						{{if $data['dealer']['shipping_policy']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-truck" aria-hidden="true"></i> Shipping Policy</h3>
							<div class="gdProfile__infoCardBody">{$data['dealer']['shipping_policy']}</div>
						</div>
						{{endif}}

						{{if $data['dealer']['return_policy']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-rotate-left" aria-hidden="true"></i> Return Policy</h3>
							<div class="gdProfile__infoCardBody">{$data['dealer']['return_policy']}</div>
						</div>
						{{endif}}

						{{if $data['dealer']['additional_notes']}}
						<div class="gdProfile__infoCard">
							<h3 class="gdProfile__infoCardTitle"><i class="fa-solid fa-note-sticky" aria-hidden="true"></i> Additional Notes</h3>
							<div class="gdProfile__infoCardBody">{$data['dealer']['additional_notes']}</div>
						</div>
						{{endif}}
					</div>
				</div>
...
{
    try { \IPS\Log::log( 'templates_10233 dealerProfile replace failed: ' . $e->getMessage(), 'gddealer_upg_10281' ); }
    catch ( \Throwable ) {}
}
