...
/**
 * v1.0.158 PART 3 of 4 - HTML body of feedSettings (read-only redesign).
 *
 * Replaces the old editable form with:
 *   - Wizard-done success flash (when ?wizard_done=1)
 *   - First-timer banner explaining where to edit vs upload
 *   - Sync health card (unchanged)
 *   - Current configuration display (read-only)
 *   - Field mapping summary (count-only, link to wizard step 3)
 *   - Manual upload widget (manual-mode only)
 *   - Recent uploads table
 *   - Recent imports log
 *
 * Initializes $feedSettingsTpl.
 */
