<?php
namespace IPS\gddealer\setup\upg_10207;
...
TEMPLATE_EOT,
		...
		foreach ( $gddealerTemplates as $tpl )
		{
			try
			{
				try
				{
					\IPS\Db::i()->delete( 'core_theme_templates', [
						...
						$tpl['app'], $tpl['location'], $tpl['group'], $tpl['template_name'], 0
					] );
				}
				catch ( \Throwable ) {}
				...
				\IPS\Db::i()->replace( 'core_theme_templates', [
					...
					'template_app'      => (string) $tpl['app'],
					'template_location' => (string) $tpl['location'],
					'template_group'    => (string) $tpl['group'],
					'template_name'     => (string) $tpl['template_name'],
					'template_data'     => (string) $tpl['template_data'],
					'template_content'  => (string) $tpl['template_content'],
					'template_updated'  => time(),
					'template_version'  => '1.0.207',
				] );
				$reseeded++;
			}
			catch ( \Throwable $e )
			{
				...
				{
					\IPS\Log::log(
						'upg_10207 replace failed for ' . ( $tpl['template_name'] ?? '?' )
						. ': ' . $e->getMessage(),
						'gddealer_upg_10207'
					);
				}
		...
		try
		{
			\IPS\Log::log( "upg_10207 complete: reseeded=$reseeded failed=$failed", 'gddealer_upg_10207' );
		}
		catch ( \Throwable ) {}
