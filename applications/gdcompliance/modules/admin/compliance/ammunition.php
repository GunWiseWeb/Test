<?php
/**
 * @brief  GD Compliance — Ammunition state-advisory dashboard (v1.6.47)
 *
 * Reads/writes gd_compliance_advisory_rules WHERE firearm_class='ammo'
 * — one row per state. Displays currently-enabled ammunition advisories with
 ...
 * differences are the WHERE clause (firearm_class = 'ammo'), the
...
class _ammunition extends \IPS\Dispatcher\Controller
{
	...
	protected function manage(): void
	{
		...
		try
		{
			foreach ( \IPS\Db::i()->select(
				...
				'gd_compliance_flags',
				[ 'firearm_type=? AND upc IN ( SELECT upc FROM ' . \IPS\Db::i()->prefix . 'gd_catalog WHERE category_id IN (23,24,25,26,27,28,29,30) )', 'advisory' ],
				null,
				null,
				'state_code'
			) as $row )
		...
		try
		{
			$distinctAdvisoryUpcs = (int) \IPS\Db::i()->select( 'COUNT(DISTINCT upc)', 'gd_compliance_flags',
				[ 'firearm_type=? AND upc IN ( SELECT upc FROM ' . \IPS\Db::i()->prefix . 'gd_catalog WHERE category_id IN (23,24,25,26,27,28,29,30) )', 'advisory' ] )->first();
		}
		catch ( \Throwable ) {}
		...
		try
		{
			foreach ( \IPS\Db::i()->select( '*', 'gd_compliance_advisory_rules', [ 'firearm_class=?', 'ammo' ],
		...
		$summary = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
			. '<h2 class="ipsType_sectionHead" style="margin:0 0 6px">' . $h( $lang->addToStack( 'gdcompliance_acp_ammo_title' ) ) . '</h2>'
			. '<p style="margin:0 0 10px;color:#475569">' . $h( $lang->addToStack( 'gdcompliance_acp_ammo_intro' ) ) . '</p>'
		...
		else
		{
			...
			foreach ( $rules as $r )
			{
				...
				$saveUrl = (string) \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=ammunition&do=save' );
		...
		try
		{
			foreach ( \IPS\Db::i()->select( 'DISTINCT state_code', 'gd_compliance_advisory_rules',
				[ 'firearm_class=?', 'ammo' ], 'state_code ASC' ) as $sc )
			{
		...
		$baseUrl = \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=ammunition' );
		...
		$catSubSelect = 'f.upc IN ( SELECT upc FROM ' . \IPS\Db::i()->prefix . 'gd_catalog WHERE category_id IN (23,24,25,26,27,28,29,30) )';
		...
		try
		{
			...
			foreach ( $sel as $r )
			{
				...
				$flaggedRowsHtml .= '<tr>'
					...
					. '<td style="padding:6px 10px;border-bottom:1px solid #f1f5f9;text-align:right"><a href="' . $h( $overrideUrl ) . '" class="ipsButton ipsButton--secondary ipsButton--verySmall">' . $h( $lang->addToStack( 'gdcompliance_acp_ammo_override' ) ) . '</a></td>'
					. '</tr>';
			}
		}
		catch ( \Throwable $e )
		...
		$flaggedTable = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
			. '<h3 style="margin:0 0 6px;font-size:14px;color:#334155">' . $h( $lang->addToStack( 'gdcompliance_acp_ammo_flagged_title' ) )
			. ' <span style="color:#64748b;font-weight:400">(' . number_format( $flagCount ) . $flaggedHeader . ')</span></h3>'
			. '<p style="margin:0 0 10px;color:#64748b;font-size:12px">' . $h( $lang->addToStack( 'gdcompliance_acp_ammo_flagged_intro' ) ) . '</p>'
			. $stateTabs
		...
		\IPS\Output::i()->title  = $lang->addToStack( 'gdcompliance_acp_ammo_title' );
	...
	protected function save(): void
	{
		...
		if ( $id <= 0 )
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=ammunition' ) );
			return;
		}
		try
		{
		...
		\IPS\Output::i()->redirect(
			\IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=ammunition' )->setQueryString( 'saved', 1 )
		);
	}
}

class ammunition extends _ammunition {}
