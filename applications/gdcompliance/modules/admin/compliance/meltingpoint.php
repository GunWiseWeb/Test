<?php
/**
 * @brief  GD Compliance — Melting-Point handgun ban dashboard (v1.6.19)
 *
 * Hybrid of the Advisories page (per-state rule editor) and Lowers page
 * (curated overrides CRUD + flagged-products workflow):
 *
 *   1. Summary ipsBox with stat cards (flagged handguns, total flag
 *      rows, active states, curated overrides).
 *   2. Per-state edit cards — enabled / threshold_f / effective_date /
 *      citation / reason text. Editable inline.
 *   3. Clickable state filter (All | HI | IL | MD | MA | MN | NY),
 *      per-state count, ?state=XX.
 *   4. Flagged-handguns table — native
 *      Db::select([table, alias], ...)->join([table, alias], ...) NEVER
 *      raw preparedQuery. Filter by state; per-row "Set override" link
 *      into the Overrides controller (upc + state pre-filled).
 *   5. Prev/Next pager preserving ?state=XX (aliased-table COUNT to
 *      avoid the v1.6.16-class silent-error bug).
 *   6. Curated overrides Table\Db (gd_compliance_melting) — add / edit
 *      / delete with force_flag / force_clear / review action. Curated
 *      entries win over auto brand matching.
 */
...
class _meltingpoint extends \IPS\Dispatcher\Controller
{
	...
	protected function manage(): void
	{
		...
		$baseUrl = \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint' );
		...
		try
		{
			foreach ( \IPS\Db::i()->select( 'state_code, COUNT(*) AS c', 'gd_compliance_flags',
				[ 'firearm_type=?', 'melting_point' ], null, null, 'state_code' ) as $row )
			{
				$perState[ (string) $row['state_code'] ] = (int) $row['c'];
			}
		}
		catch ( \Throwable ) {}
		$totalMpFlags = array_sum( $perState );

		$distinctFlagged = 0;
		try
		{
			$distinctFlagged = (int) \IPS\Db::i()->select( 'COUNT(DISTINCT upc)', 'gd_compliance_flags',
				[ 'firearm_type=?', 'melting_point' ] )->first();
		}
		catch ( \Throwable ) {}
		...
		try
		{
			$curatedCount = (int) \IPS\Db::i()->select( 'COUNT(*)', 'gd_compliance_melting' )->first();
		}
		catch ( \Throwable ) {}
		...
		try
		{
			foreach ( \IPS\Db::i()->select( '*', 'gd_compliance_melting_rules', null, 'state_code ASC' ) as $row )
			{
				$rules[] = $row;
			}
		}
		...
		if ( $saved )
		{
			$savedBanner = '<div class="ipsBox" style="margin-bottom:14px;border-left:4px solid #059669"><div class="ipsBox_body ipsPad" style="background:#ecfdf5"><strong style="color:#065f46">Saved.</strong> Recompute to re-emit melting_point flags with the updated text.</div></div>';
		}

		$summary = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
			. '<h2 class="ipsType_sectionHead" style="margin:0 0 6px">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_title' ) ) . '</h2>'
			. '<p style="margin:0 0 10px;color:#475569">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_intro' ) ) . '</p>'
			. '<div style="display:flex;gap:24px;flex-wrap:wrap;font-size:13px">'
			. '<div><strong style="color:#0f172a;font-size:22px">' . number_format( $distinctFlagged ) . '</strong><br><span style="color:#64748b">flagged handguns</span></div>'
			. '<div><strong style="color:#0f172a;font-size:22px">' . number_format( $totalMpFlags ) . '</strong><br><span style="color:#64748b">total flag rows</span></div>'
		...
		/* ------- Per-state edit cards (like advisories.php) ------- */
		$csrf = (string) \IPS\Session::i()->csrfKey;
		$editCards = '';
		if ( empty( $rules ) )
		{
			$editCards = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad" style="text-align:center;color:#94a3b8">No melting-point rules seeded yet. Run upg_10619 to populate HI / IL / MD / MA / MN / NY.</div></div>';
		}
		else
		{
			foreach ( $rules as $r )
			{
				...
				$saveUrl = (string) \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint&do=saveRule' );
				...
				$editCards .= '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
					...
					. '<h3 style="margin:0;font-size:15px;color:#0f172a"><span style="display:inline-block;padding:2px 10px;border-radius:999px;font-size:11px;font-weight:700;background:#fee2e2;color:#991b1b;margin-right:8px">' . $h( $state ) . '</span> ' . $h( $stateName ) . ' <span style="color:#64748b;font-size:12px;font-weight:400">&middot; melting-point ban</span></h3>'
					...
					. '</select></label>'
					. '<label style="display:flex;flex-direction:column;gap:3px;font-size:12px"><span>Threshold °F</span><input type="number" name="threshold_f" min="500" max="2000" value="' . (int) $thresh . '"></label>'
					...
					. '<label style="display:flex;flex-direction:column;gap:3px;font-size:12px"><span>Citation</span><input type="text" name="citation" maxlength="255" value="' . $h( $cite ) . '"></label>'
					. '<label style="display:flex;flex-direction:column;gap:3px;font-size:12px;grid-column:span 2"><span>Customer-visible reason</span><textarea name="reason" rows="4">' . $h( $reason ) . '</textarea><span style="color:#64748b;font-size:11px">Shown in the red restriction popup on the product page.</span></label>'
					. '</div>'
		...
		$stateTabs .= '<a class="ipsButton ipsButton--verySmall' . $allActive . '" href="' . $h( (string) $baseUrl ) . '">All (' . number_format( $totalMpFlags ) . ')</a>';
		...
		{
			try { \IPS\Log::log( 'meltingpoint flagCount: ' . $e->getMessage(), 'gdcompliance' ); } catch ( \Throwable ) {}
		}
		...
		try
		{
			...
			foreach ( $sel as $r )
			{
				...
				$rowsHtml .= '<tr>'
					...
					. '<td style="padding:6px 10px;border-bottom:1px solid #f1f5f9;text-align:right"><a href="' . $h( $overrideUrl ) . '" class="ipsButton ipsButton--secondary ipsButton--verySmall">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_override' ) ) . '</a></td>'
					. '</tr>';
			}
		}
		catch ( \Throwable $e )
		{
			try { \IPS\Log::log( 'meltingpoint flagged list: ' . $e->getMessage(), 'gdcompliance' ); } catch ( \Throwable ) {}
		}

		if ( $rowsHtml === '' )
		{
			$rowsHtml = '<tr><td colspan="6" style="padding:20px;text-align:center;color:#94a3b8">No melting-point flags'
		...
		$flaggedTable = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
			. '<h3 style="margin:0 0 6px;font-size:14px;color:#334155">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_flagged_title' ) )
			. ' <span style="color:#64748b;font-weight:400">(' . number_format( $flagCount )
			. ( $stateFilter !== '' ? ' in ' . $h( $stateFilter ) : '' ) . ')</span></h3>'
			. '<p style="margin:0 0 10px;color:#64748b;font-size:12px">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_flagged_intro' ) ) . '</p>'
			. $stateTabs
		...
		$curatedTable = new \IPS\Helpers\Table\Db( 'gd_compliance_melting', $baseUrl );
		$curatedTable->langPrefix    = 'gdcompliance_acp_mp_col_';
		...
		$curatedTable->rowButtons = function( $row ) {
			$base = 'app=gdcompliance&module=compliance&controller=meltingpoint';
			return [
		...
		$addUrl       = (string) \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint&do=form' );
		$curatedIntro = '<div class="ipsBox" style="margin-bottom:14px"><div class="ipsBox_body ipsPad">'
			. '<h3 style="margin:0 0 6px;font-size:14px;color:#334155">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_curated' ) ) . '</h3>'
			. '<p style="margin:0 0 10px;color:#475569;font-size:13px">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_curated_intro' ) ) . '</p>'
			. '<a href="' . $h( $addUrl ) . '" class="ipsButton ipsButton--primary ipsButton--small">' . $h( $lang->addToStack( 'gdcompliance_acp_mp_add' ) ) . '</a>'
			. '</div></div>';

		\IPS\Output::i()->title  = $lang->addToStack( 'gdcompliance_acp_mp_title' );
	...
	protected function saveRule(): void
	{
		...
		if ( $id <= 0 )
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint' ) );
			return;
		}
		try
		{
			...
			\IPS\Db::i()->update( 'gd_compliance_melting_rules', [
				...
				'threshold_f'    => max( 500, min( 2000, (int) ( \IPS\Request::i()->threshold_f ?? 800 ) ) ),
				'reason'         => trim( (string) \IPS\Request::i()->reason ),
				'citation'       => substr( (string) \IPS\Request::i()->citation, 0, 255 ) ?: null,
				'effective_date' => $eff,
				'updated_at'     => time(),
			], [ 'id=?', $id ] );
			try { \IPS\gdcompliance\MeltingPoint::clearCache(); } catch ( \Throwable ) {}
		}
		catch ( \Throwable $e )
		{
			try { \IPS\Log::log( 'meltingpoint saveRule: ' . $e->getMessage(), 'gdcompliance' ); } catch ( \Throwable ) {}
		}
		\IPS\Output::i()->redirect(
			\IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint' )->setQueryString( 'saved', 1 )
		);
	}
	...
	protected function form(): void
	{
		...
		if ( $id > 0 )
		{
			try { $row = \IPS\Db::i()->select( '*', 'gd_compliance_melting', [ 'id=?', $id ] )->first(); }
			catch ( \Throwable ) { $row = null; }
		}

		$form = new \IPS\Helpers\Form;
		$form->add( new \IPS\Helpers\Form\Text(   'gdcompliance_mp_f_pattern', $row['pattern'] ?? '', TRUE,  [ 'maxLength' => 191 ] ) );
		$form->add( new \IPS\Helpers\Form\Select( 'gdcompliance_mp_f_action',  $row['action']  ?? 'force_flag', TRUE, [ 'options' => self::ACTIONS ] ) );
		$form->add( new \IPS\Helpers\Form\Text(   'gdcompliance_mp_f_note',    $row['note']    ?? '', FALSE, [ 'maxLength' => 255 ] ) );

		if ( $values = $form->values() )
		{
			$pattern = trim( (string) $values['gdcompliance_mp_f_pattern'] );
			$action  = (string) $values['gdcompliance_mp_f_action'];
			...
			else
			{
				$data = [
					...
					'action'     => $action,
					'note'       => substr( (string) $values['gdcompliance_mp_f_note'], 0, 255 ) ?: null,
					'created_at' => time(),
				];
				try
				{
					if ( $row ) { \IPS\Db::i()->update( 'gd_compliance_melting', $data, [ 'id=?', $id ] ); }
					else        { \IPS\Db::i()->insert( 'gd_compliance_melting', $data ); }
					try { \IPS\gdcompliance\MeltingPoint::clearCache(); } catch ( \Throwable ) {}
				}
				catch ( \Throwable $e )
				{
					try { \IPS\Log::log( 'meltingpoint save curated: ' . $e->getMessage(), 'gdcompliance' ); } catch ( \Throwable ) {}
				}

				\IPS\Output::i()->redirect(
					\IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint' ),
					'saved'
				);
				return;
			}
		...
		\IPS\Output::i()->title  = \IPS\Member::loggedIn()->language()->addToStack( 'gdcompliance_acp_mp_add' );
	...
	protected function delete(): void
	{
		...
		if ( $id > 0 )
		{
			try
			{
				\IPS\Db::i()->delete( 'gd_compliance_melting', [ 'id=?', $id ] );
				try { \IPS\gdcompliance\MeltingPoint::clearCache(); } catch ( \Throwable ) {}
			}
			catch ( \Throwable ) {}
		}
		\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=gdcompliance&module=compliance&controller=meltingpoint' ), 'deleted' );
	}
}
