<?php
/**
 * @brief  gdsearch — upgrade 1.0.89 (re-sync all templates from dev/html/ — fixes blank price alerts + wishlist pages — 5 missing template rows).
 *
 * Same helper pattern shipped for gdloadout v1.0.78, gddeals v1.0.63,
 * and gddealer v1.0.340: walk dev/html/{location}/{group}/{name}.phtml
 * and \IPS\Db::i()->replace() the row into core_theme_templates.
 * 9-column canonical shape (matches gddealer/setup/install.php:5230)
 * — no template_master_key, no template_has_hookpoints, let IPS
 * provide defaults.
 *
...
namespace IPS\gdsearch\setup\upg_10089;
		...
		if ( is_dir( $root ) )
		{
			try
			{
				...
				foreach ( $it as $f )
				{
					...
					{
						try { \IPS\Log::log( 'upg_10089 tpl (' . $name . '): ' . $e->getMessage(), 'gdsearch_upg_10089' ); } catch ( \Throwable ) {}
					}
				}
			}
			catch ( \Throwable $e )
			{
				try { \IPS\Log::log( 'upg_10089 tpl loop: ' . $e->getMessage(), 'gdsearch_upg_10089' ); } catch ( \Throwable ) {}
			}
		}
