<?php
namespace IPS\gdcatalog\setup\upg_10098;
...
class _upgrade
{
	public function step1(): bool
	{
		...
		/* v1.0.98: Seed Scope Rings / Upper Receivers / Lower Receivers
		   as children of Parts & Accessories (idempotent — only if missing) */
		try
		{
			$parentId = (int) $db->select( 'id', 'gd_categories', [ 'name=?', 'Parts & Accessories' ] )->first();
		}
		catch ( \Throwable )
		{
			$parentId = 0;
		...
		if ( $parentId > 0 )
		{
			$newChildren = [ 'Scope Rings', 'Upper Receivers', 'Lower Receivers' ];
			foreach ( $newChildren as $childName )
			{
				try
				{
