<?php
namespace IPS\gdcatalog\setup\upg_10013;
...
class _upgrade
{
	...
	{
		/* gdcatalog v1.0.13 - Fix filter form on productList template.
		 *
		 * Bug: clicking "Filter" on the Product Browser redirected to the
		 * IPS admin dashboard instead of filtering the products list.
		 *
		 * Root cause: the filter form uses method="get" with action set to
		 *   /admin/?app=gdcatalog&module=catalog&controller=products
		 * When the browser submits a GET form, it STRIPS the action's query
		 * string and replaces it with the form's serialized fields. So the
		 * resulting URL becomes:
		 *   /admin/?q=&status=&category=1
		 * Missing app/module/controller params -> IPS routes to default
		 * admin dashboard.
		 *
		 * Fix: add hidden inputs for app/module/controller inside the form.
		 * The browser includes them in the GET submission, preserving the
		 * routing params. Standard HTML pattern.
		 *
		 * This version reseeds the productList template with the corrected
		 * filter form. All other v1.0.12 fixes (template_set_id=1,
		 * parameter order, {$pagination|raw}) are preserved.
		 *
		...
		try
		{
			...
			$msg = sprintf(
				'gdcatalog v1.0.13 sanity (pre-version-write): app_long_version=%d, app_version=%s',
				$longVer,
				(string) ( $row['app_version'] ?? '' )
			);
			try { \IPS\Log::log( $msg, 'gdcatalog_upg_10013' ); } catch ( \Throwable ) {}

			if ( $longVer < 10012 )
			{
				$warning = sprintf(
					'gdcatalog v1.0.13 WARNING: app_long_version=%d below 10012',
					$longVer
				);
				try { \IPS\Log::log( $warning, 'gdcatalog_upg_10013' ); } catch ( \Throwable ) {}
			}
		}
		catch ( \Throwable $e )
		{
			try { \IPS\Log::log( 'gdcatalog v1.0.13 sanity check failed: ' . $e->getMessage(), 'gdcatalog_upg_10013' ); } catch ( \Throwable ) {}
		}

		/* Step 2: Reseed productList template with hidden inputs in filter form. */
...
TEMPLATE_EOT;
		...
		try
		{
			...
			try { \IPS\Log::log( 'gdcatalog v1.0.13 productList template reseeded with filter form hidden inputs', 'gdcatalog_upg_10013' ); } catch ( \Throwable ) {}
		}
		catch ( \Throwable $e )
		{
			try { \IPS\Log::log( 'gdcatalog v1.0.13 productList template reseed FAILED: ' . $e->getMessage(), 'gdcatalog_upg_10013' ); } catch ( \Throwable ) {}
		}
	...
	{
		return 'gdcatalog v1.0.13 - fix Product Browser filter form (hidden inputs for app/module/controller)';
	}
}
