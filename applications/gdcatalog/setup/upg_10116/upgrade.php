<?php
/**
 * @brief  gdcatalog — upgrade 1.0.116 (re-sync all templates from dev/html/ — 3 missing template rows silently break admin flows).
 *
...
namespace IPS\gdcatalog\setup\upg_10116;
		...
		if ( is_dir( $root ) )
		{
			try
			{
				...
				foreach ( $it as $f )
				{
					...
					{
						try { \IPS\Log::log( 'upg_10116 tpl (' . $name . '): ' . $e->getMessage(), 'gdcatalog_upg_10116' ); } catch ( \Throwable ) {}
					}
				}
			}
			catch ( \Throwable $e )
			{
				try { \IPS\Log::log( 'upg_10116 tpl loop: ' . $e->getMessage(), 'gdcatalog_upg_10116' ); } catch ( \Throwable ) {}
			}
		}
