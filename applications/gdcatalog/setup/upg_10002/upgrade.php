...
class _upgrade
{
	public function step1(): bool
	{
		...
		/* 1. Reseed feedList template (replace existing v1.0.0 row at template_set_id=1) */
		try
		{
		...
		{
			try { \IPS\Log::log( 'v1.0.2 feedList reseed failed: ' . $e->getMessage(), 'gdcatalog_upg_10002' ); } catch ( \Throwable ) {}
			return FALSE;
		}

		/* 2. Insert new lang strings into core_sys_lang_words for every installed language.
		 *    lang.xml is only parsed on fresh install; running upgrades require explicit DB inserts.
		 *    Schema columns match the proven-working gddealer upgrade pattern (upg_10077, upg_10082, etc.). */
		$newStrings = [
		...
		try
		{
			...
			foreach ( $languages as $langId )
			{
				foreach ( $newStrings as $key => $val )
				{
					...
					{
						try { \IPS\Log::log( 'v1.0.2 lang seed row failed key=' . $key . ' lang_id=' . $langId . ': ' . $rowException->getMessage(), 'gdcatalog_upg_10002' ); } catch ( \Throwable ) {}
						/* Continue loop — one row failure shouldn't poison the rest */
					}
				}
			}
		}
		...
		{
			try { \IPS\Log::log( 'v1.0.2 lang seed outer failed: ' . $e->getMessage(), 'gdcatalog_upg_10002' ); } catch ( \Throwable ) {}
			/* Outer catch is for the SELECT failing — non-fatal, don't break upgrade */
		}
	...
	{
		return 'v1.0.2 - reseed feedList template + insert lang strings for add/delete/reorder UI';
	}

	/**
	 * v1.0.2 feedList template content. Inlined per project rule #28 (no
